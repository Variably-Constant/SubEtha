...
- [Polymorphic substrate](#polymorphic-substrate-locale-x-protocol-x-shape-x-capacity-x-ordering)
...
| [`subetha-ffi`](crates/subetha-ffi) | The C ABI. One header, `subetha.h`, over generation-checked handles, every panic caught at the boundary. `cargo add subetha-ffi` for a Rust caller; a C caller builds the shared or static library and takes the committed header. The order in which the C surface ships is in [`C_ABI_TIERS.md`](C_ABI_TIERS.md). |
