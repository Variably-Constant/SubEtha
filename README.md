...
| [`ReliableUdpSender`](crates/subetha-cxc/src/udp_bridge.rs) / `ReliableUdpReceiver` (`SensOMaticRs*`) | Sens-O-Matic transport, block Reed-Solomon code (FEC + ARQ, no TLS) | `subetha_cxc::udp_bridge` |
| [`SensOMaticRlcSender`](crates/subetha-cxc/src/sens_rlc.rs) / `SensOMaticRlcReceiver` | Sens-O-Matic transport, sliding-window RLC code (FEC + ARQ, optional TLS) | `subetha_cxc::sens_rlc` |
...
- **Cross-host pipelines** that otherwise require a separate network-channel design. `QuicBridgeClient`/`QuicBridgeServer` (Cargo feature `quic-bridge`) ferry bytes between two `AdaptiveRing` instances on different hosts via real QUIC streams with TLS-via-rustls (~490K items/s on 127.0.0.1 in [`quic_bridge_e2e`](crates/subetha-cxc/examples/quic_bridge_e2e.rs)). `TcpBridgeClient`/`TcpBridgeServer` (Cargo feature `tcp-bridge`) do the same over plain TCP for trusted networks and, without the TLS+async overhead QUIC carries, run several times faster (~4.5 M items/s on loopback), and the **Sens-O-Matic** reliable-FEC protocol (no extra deps; optional `tls`) holds throughput and a low latency tail on lossy or wireless links where the TCP bridges collapse. All compose on the same shape-axis morphing that drives local IPC.
...
A channel is a frozen handshake between two roles. `std::sync::mpsc::channel` freezes "single process, in memory" into the channel. `socket(AF_UNIX, SOCK_STREAM, 0)` freezes "kernel-mediated byte streams" into the channel. `TcpStream` freezes the entire IP stack into the channel. Those choices are correct for the median workload. They are wrong wherever the topology crosses a process boundary AND the application wants in-process speed.

CXC breaks the freeze in two directions. They are orthogonal.

**By topology.** Lift the channel into a memory-mapped file. Same primitive serves cross-thread, cross-process, and disk-persistent without a rewrite. The kernel is on the path at `mmap()` and never again. Cross-machine extends through a bridge at the edge - QUIC, plain or TLS TCP, or the Sens-O-Matic reliable-FEC protocol, picked by link trust and quality - so both sides keep their fast local path (see [Going cross-host](#going-cross-host)).

**By workload.** Make the dispatch end of the channel a runtime field. An `AtomicU32` tag indexes into pre-allocated backings (SharedRing for streaming MPMC, SharedDeque variants for work-stealing, SharedHashMap for key-value). `MmfDispatcher` consults a declarative workload signature at construction time and picks the empirically-best primitive without consumer involvement. The tag can flip live without losing items, because both backings stay pre-allocated and the migration is one Release-store on a shared atomic.

One philosophy, two un-freezings. That is the whole point.

**Cross-process wake without per-message syscalls.** The waker is the userspace `futex` idea, ported to a wake-list inside the MMF, so it works on every OS that can map a file. A consumer parks on a registered sequence; the producer's send overtakes it and wakes exactly that consumer. Sends with nobody parked cost one cache line. A single parked-mask word in the header answers the common case before any slot is touched.

What about the waits that do park? Before touching the kernel, every wait arms a hardware monitor on the slot's cache line (`MONITORX`/`MWAITX` or `UMONITOR`/`UMWAIT` on x86-64, `LDAXR`+`WFE` on aarch64, probed at runtime) and light-sleeps there. The producer's own store is the wake. No syscall on either side, and it measures 120 ns median against 9.3 us for the kernel park on the same Zen+ host. This tier is also what gives Windows a true cross-process wake: `WaitOnAddress` cannot cross a process boundary, but a hardware monitor keys on physical addresses and does not care who stored. Each platform's kernel-park arm behind the monitor tier is detailed in [`docs/LOW_LEVEL_OPTIMIZATIONS.md`](docs/LOW_LEVEL_OPTIMIZATIONS.md).

The waker itself is `crates/subetha-cxc/src/cross_process_waker.rs`. The blocking rings (`BlockingSpscRing`, `BlockingMpscRing`, `BlockingMpmcRing`), `SharedCondvar`, `BlockingSemaphore`, `BlockingRWLock`, and the async adapter `AsyncSpscRing` all layer on top.
...
|---|---|---|---|---|
| `QuicBridge` | `quic-bridge` | QUIC over UDP | TLS 1.3 via rustls | Untrusted networks. Multiplexed streams (one per `Channel<T>`, no cross-channel head-of-line blocking), connection migration (peers change IPs without losing the channel), 0-RTT resumption (cheap reconnect after a crash, matching MMF's re-`mmap()` recovery story). BBR-style control holds throughput under loss where TCP backs off. |
...
| **Sens-O-Matic** | none (`std`) / `tls` | Reliable UDP, FEC-primary + ARQ-fallback | none, or TLS 1.3 (RLC code) | The reliable FEC-UDP **protocol** (the QUIC peer of this set), for lossy or wireless links. Forward error correction recovers most loss with no retransmit round-trip; ARQ is the floor. The erasure code is swappable, like a cipher suite: the **block Reed-Solomon** code (`ReliableUdpSender`/`Receiver`, also `SensOMaticRs*` - MDS, systematic Cauchy, transmit interleaving, `std`-only) or the **sliding-window RLC** code (`SensOMaticRlc*` in `sens_rlc` - adaptive, packet-pair rate control, optional TLS 1.3). The parity rate auto-adapts to the loss the receiver measures, and a link sensor anticipates it. A unified endpoint (`UnifiedSens*` in `sens_unified`) carries both codes on one port and switches between them mid-stream at the ~15% loss crossover (RLC below for latency, RS above for throughput). |
