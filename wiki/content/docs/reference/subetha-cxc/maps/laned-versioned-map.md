...
> [Shared BTree Map](../shared-btree-map/) is single-writer - two
> simultaneous writers mutate node structure under one seqlock and
> corrupt it - and [Versioned BTree Map](../versioned-btree-map/)
...
[Shared Epochs](../../coordination-types/shared-epochs/), plus a
[Holder Table](../../coordination-types/holder-table/) of lane claims,
so the plain versioned map is untouched and no existing user pays for
lanes.
...
### Pattern: many statements writing one index

A store admitting `n` concurrent writing statements gives each a lane
for its duration. Where a single tree would have them queue on one
mutex, they run at once, and a scan still reads one ordered view.

### Pattern: records beside the index

The rows these keys name live in a
[Shared Versioned Slab](../../specialized/shared-versioned-slab/) on
the same epoch table. The index says which keys a scan sees; the slab
says which version of each record; one horizon reclaims both.
...
- Substrate: [Versioned BTree Map](../versioned-btree-map/) - the map a
  lane is, unchanged.
- Substrate: [Shared Epochs](../../coordination-types/shared-epochs/) -
  the counter, the pins and the tickets every lane shares.
- Substrate: [Holder Table](../../coordination-types/holder-table/) -
  the lane claims, and the dead-holder recovery behind
  `reap_dead_claims`.
- Sibling primitive:
