...
**Composed, not folded in.** A [Shared BTree Map](../shared-btree-map/)
of `Versioned<V>` beside a
[Shared Epochs](../../coordination-types/shared-epochs/), so the plain
tree is untouched and no existing user pays for versioning.
...
- **An update replaces the entry in place.** One entry per key means
  ...
  record ([Shared Versioned Slab](../../specialized/shared-versioned-slab/))
  and uses this map for keys that are born and die but do not change,
  such as a composite of key and record id.
...
nothing reporting it. `insert` returns `VersionedError::RebornUnderPin`
rather than take that trade.
...
### Pattern: a scan that must not block writers

An index walked by a long analytical scan while ingest continues.
The scan pins once at its start and every writer keeps running; the
cost is the tombstones created during the walk.

### Pattern: one counter for records and index
...
- **Bounded nodes at create**: no auto-grow.
- **A rebirth under a live pin is refused**, not queued.
- **`len()` includes tombstones**, so it is not a live-entry count.
...
- Substrate: [Shared BTree Map](../shared-btree-map/) - the ordered map
  it holds, unchanged.
- Substrate: [Shared Epochs](../../coordination-types/shared-epochs/) -
  the counter and the pins.
- Sibling primitive:
