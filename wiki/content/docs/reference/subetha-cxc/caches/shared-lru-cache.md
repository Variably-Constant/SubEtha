...
[`SharedHashMap<K, u32>`](../maps/shared-hash-map/) for O(1) lookup
with [`SharedLinkedList<(K, V)>`](../maps/shared-linked-list/) for
...
```mermaid
flowchart LR
    M["SharedHashMap of K to u32<br/>K maps to handle.as_u32"]
    L["SharedLinkedList of (K, V)<br/>handle maps to Node; head = MRU, tail = LRU"]
    M -- handle lookup --> L
    classDef mapC fill:#1e3a8a,color:#ffffff
    classDef listC fill:#0f766e,color:#ffffff
    class M mapC
    class L listC
```
...
- Source: `crates/subetha-cxc/src/shared_lru_cache.rs` (~600
  lines, unit tests covering put/get/touch/evict, eviction
