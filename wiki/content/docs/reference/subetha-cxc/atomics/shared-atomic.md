...
- **All standard atomic ops shipped**: `load`, `store`, `fetch_add`,
  ...
  `compare_exchange` (source lines 99-144 for U32/U64).
  `SharedAtomicBool` ships `load`, `store`, `swap` (source lines
  214-216) - no fetch_add semantics for bool.
- **`flush` / `flush_async` for durability**: `flush` calls
  ...
  (page-cache only).
- **Send + Sync** unconditional (source lines 53-54, 171-172).
