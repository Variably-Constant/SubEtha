...
- **`T: Copy + 'static` required.** This is the struct bound on
  `SharedUmbraPointer<T>`; `SharedRegion<T>` carries the same
...
- **16-byte alignment is REQUIRED.** `#[repr(C, align(16))]` on
  ...
  the prefix being at a stable byte offset.
...
- **`ext<E>()` is `unsafe`.** The tag-uniqueness contract is on
  the caller; two `UmbraExtension` implementations sharing a TAG
  silently misinterpret each other's payloads. Validate TAG
  uniqueness at the application level (e.g. a doc comment listing
  claimed tags). (`set_ext` is safe; only the read side is
  `unsafe` because it reinterprets payload bytes as `E`.)
- **Extension size limit: 7 bytes.** The `ExtSizeCheck<E>::CHECK`
  const-assertion (monomorphized by both `set_ext` and `ext`)
  fails to compile if `size_of::<E>() > 7`. Use a wrapper or
  store an OffsetPtr to a larger value.
- **`resolve` touches the region MMF.** One region read per call
...
- **Content-prefix is endian-dependent.** `content_prefix_of`
...
- **Don't use a Wide T directly as `OffsetPtr::T` if the region
  can't hold it.** `SharedRegion<T>` requires `T: Copy +
  'static` and copies `T` into fixed-size slots; non-Copy or
  unbounded types need to be stored elsewhere with the OffsetPtr
  pointing into a separate region.
