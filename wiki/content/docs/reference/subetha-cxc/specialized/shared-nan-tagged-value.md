...
- **Layout-compatible superset of `SharedNaNValue`.** Source
  lines 59-66. Every `SharedNaNValue` lifts losslessly into
...
- **`TaggedOffsetPtr<T, TAG_BITS>` const generic must be known
  at extraction.** Source line 170. The `T` and `TAG_BITS`
...
- **3-bit outer tag = 8 slots; 6 boxed tags used (0 nil, 1 i32,
  2 u32, 3 bool, 4 OffsetPtr, 5 TaggedOffsetPtr), tags 6 and 7
  reserved** (f64 is the unboxed case, not a tag). `SharedNaNValue`
  defines tag 5 as `TAG_TAGGED_OFFSET_PTR` and this struct is the
  type that consumes it.
- **48-bit payload, of which the low 32 bits hold
  `TaggedOffsetPtr::raw()`.** Source line 101 masks to 48 bits
...
block-beta
  columns 4
  s["bit 63: sign = 1"] e["bits 62..52: exp 0x7FF (all ones)"] q["bits 51..48: qNaN = 1 + outer tag (3 bits)"] p["bits 47..0: payload (48 bits)"]
  classDef signC fill:#9a3412,color:#ffffff
  classDef expC fill:#1e3a8a,color:#ffffff
  classDef tagC fill:#475569,color:#ffffff
...
block-beta
  columns 2
  t["inner tag: N bits"] i["index: 32 - N bits"]
  classDef tagC fill:#9a3412,color:#ffffff
  classDef idxC fill:#0f766e,color:#ffffff
  class t tagC
  class i idxC
```
...
The discriminator type for `type_tag()` is `NaNTaggedType` (source
line 187), a strict superset of `NaNValueType` adding the
...
- **`(T, TAG_BITS)` discipline is the caller's job.** Source
  line 170. Mismatched const generics at construction vs
  extraction yield a `TaggedOffsetPtr` with the wrong tag /
  index split. The slot itself just holds a u32.
- **Outer tag 5 is now taken.** Source line 75. Only tags 6
  and 7 remain reserved as extension points for the NaN-box
  family.
- **Downcast to `SharedNaNValue` loses the discriminator.**
  Source lines 114-116. The bits round-trip but the type is
...
- **Construction does not validate `TaggedOffsetPtr` invariants.**
  Source line 101 packs the raw u32 directly. If the
