...
- **3-bit tag field (8 slots; 6 used, 2 reserved).** Source lines
  22-32. Adding a new tag value requires reserving one of the
  remaining slots; no expansion path beyond 8.
- **48-bit payload.** Source line 62. `i32` and `u32` use 32 bits;
  `OffsetPtr` uses 32 bits (max region size = 4 G slots);
  `bool` uses 1 bit; remaining payload bits are zero.
- **`OffsetPtr<T>` is type-erased in the slot.** Source lines
  135-137. The `T` parameter is dropped at construction; callers
...
- **No payload type check on extraction.** Source lines 183-207.
...
- **`#[repr(C)]` 8-byte slot.** Source line 92. Layout-stable for
  cross-process bit patterns.
- **Boolean payload uses only the lowest bit** (source line 200).
...
block-beta
  columns 4
  s["bit 63: sign = 1"] e["bits 62..52: exp 0x7FF (all ones)"] q["bits 51..48: qNaN = 1 + tag (3 bits)"] p["bits 47..0: payload (48 bits)"]
...
`SharedNaNValue` derives `Copy + Clone + Debug + PartialEq + Eq +
Hash` (no `Ord`/`PartialOrd`), so it works as a `HashMap` value or
`HashSet` element but not directly as a `BTreeMap`/`BTreeSet` key.
...
- **NaN payloads are LOSSY.** `from_f64(NaN)` canonicalises to
  `0x7FF8_0000_0000_0000` (source line 111-115). The specific NaN
  bit pattern (sNaN vs qNaN, NaN payload bits) is not preserved.
  Test `f64_nan_canonicalised` exercises this.
- **OffsetPtr's `T` parameter is type-erased.** Source line 135.
...
- **3-bit tag field = 8 slots, 2 reserved.** Source lines 22-32.
  Adding a 9th value type requires restructuring the
  encoding.
- **48-bit payload caps `OffsetPtr` index at 2^32 (u32 max).**
  Source line 137: `p.index as u64`. The full 48-bit payload is
...
- **Reserved tags decode as `NaNValueType::Reserved(u64)`.** Source
  lines 167-169. No accessor is exposed for reserved-tag payloads;
