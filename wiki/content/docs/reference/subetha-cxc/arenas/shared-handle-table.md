...
- **`SLOT_PAYLOAD_BYTES = 48`** (source line 57): per-slot payload
  capacity (slot is 64-byte cache line minus header fields).
- **Handle packing** (rustdoc lines 4-8): u64 = `(gen: u32 << 32) | slot: u32`.
  `Handle::NULL = 0` is the reserved sentinel.
- **Generation parity** (rustdoc lines 32-37): even = vacant,
  odd = occupied. Bumped on every insert and every remove.
- **ABA-free Treiber stack free list** (rustdoc lines 39-45):
  head packed as `(counter: u32, slot_idx: u32)`. CAS bumps the
  counter on every push/pop.
- **Each slot is 64 bytes** (source line 75: `#[repr(C, align(64))]`).
...
- Source: `crates/subetha-cxc/src/shared_handle_table.rs` (with unit
  tests covering insert/get/remove, stale rejection, full-table
