...
defined tag into the high bits of an [`OffsetPtr`](offset-ptr/)
...
- **Pointer encoding only, not an allocator.** Pair with a
  [`SharedRegion`](offset-ptr/) or any slot-indexed structure.
...
not a container. Use it alongside [`SharedRegion`](offset-ptr/)
