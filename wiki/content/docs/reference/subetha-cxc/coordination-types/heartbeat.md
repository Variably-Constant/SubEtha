...
block-beta
  columns 1
  hdr["HeartbeatHeader - 64 B: magic, capacity, epoch AtomicU64 (watchdog ticks; everyone reads)"]
  s0["HeartbeatSlot 0 - 64 B (one cache line each): pid AtomicU32, seq_version AtomicU32, last_seen_epoch u64, in_flight_bitmap u64, role AtomicU32"]
  s1["HeartbeatSlot 1 - same shape"]
  dots["..."]
  sn["HeartbeatSlot N - 1"]
...
bumped only by the two structural lifecycle ops, `register` and
`unregister` (each bumps it to odd then back to even around the
slot's multi-field rewrite). Every other write is a single atomic
word: `beat` stores `last_seen_epoch`, and `mark_in_flight` /
`clear_in_flight` / the watchdog's `clear_dead_bitmap` reclamation
are RMW / store on `in_flight_bitmap` - NONE of them bump
`seq_version`. A reader sees each of those as a single atomic load
with no field-level tear; the SeqLock exists only to protect the
multi-field register/unregister rewrite from being read torn.
...
- Source: `crates/subetha-cxc/src/heartbeat.rs` (376 lines, 6 unit
  tests covering register, table-full, beat advancing
  last_seen_epoch, unregister reuse, in-flight bitmap mark+clear,
  and SeqLock snapshot consistency). Also exposes `capacity()`,
