...
- **Two constructors**: `FailoverWatchdog::new(table)` uses the
  `DEFAULT_GRACE_EPOCHS` constant (= 1); `with_grace(table,
  grace_epochs)` sets it explicitly.
...
flowchart BT
    ...
    classDef tblC fill:#1e3a8a,color:#ffffff
    class W wdC
    class T tblC
```
...
- Source: `crates/subetha-cxc/src/failover.rs` (186 lines, 4 unit
  ...
  silencing). `ReclaimReport::is_empty()` is the quick "any dead?"
  check; `FailoverWatchdog` / `ReclaimReport` / `DEFAULT_GRACE_EPOCHS`
