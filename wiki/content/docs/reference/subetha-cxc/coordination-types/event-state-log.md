...
view. Composes [`SharedRing`](../rings/shared-ring/) (the durable
event log) with [`SharedCell`](../cells/shared-cell/) (the
...
```mermaid
flowchart BT
    H["all process / thread handles"]
    E["base.events.bin<br/>SharedRing, Vyukov MPMC slots"]
    S["base.state.bin<br/>SharedCell of State, SeqLock + 52 B payload"]
    H -- "emit(event): one try_push" --> E
    H -- "drain_and_fold pops + folds" --> E
    H -- "set(state) after fold" --> S
    H -- "read_current() hot path" --> S
    classDef ringC fill:#0f766e,color:#ffffff
    classDef cellC fill:#1e3a8a,color:#ffffff
...
1. Serialize `event` into the ring's 56-byte payload via
...
`EventLogError` has four variants: `Ring(RingError)` (propagated from the
...
to disk (msync), making the ring file the authoritative on-disk event log;
`flush_async()` is the non-blocking variant that delegates to each component's
async flush. On Windows the async path is only partially asynchronous (it
syncs to the page cache, not all the way to disk).
...
- Source: `crates/subetha-cxc/src/event_state_log.rs` (370 lines, 9
