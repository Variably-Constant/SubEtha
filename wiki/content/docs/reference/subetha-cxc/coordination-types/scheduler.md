...
Autonomous Pass executor backed by [`SharedRing`](../rings/shared-ring/)
+ [`HeartbeatTable`](heartbeat/) +
[`FailoverWatchdog`](failover/) +
[`pass_registry`](pass-registry/). Each process opens shared
...
```mermaid
flowchart TB
    SUB["Submitter: submit(pass) gives result_token"]
    SR["SharedRing submit - base.submit.bin"]
    W["Worker thread (per process):<br/>1 drain submit ring, 2 mark in_flight bit,<br/>3 pass_registry execute, 4 push to result ring,<br/>5 clear in_flight bit, 6 heartbeat.beat"]
    RR["SharedRing result - base.result.bin"]
    RC["ResultCollector: try_recv() gives SubmittedResult"]
    HB["HeartbeatTable - base.hb.bin"]
    FW["FailoverWatchdog (coordinator only):<br/>scan() gives ReclaimReport"]
    SUB --> SR --> W --> RR --> RC
    W --> HB
    FW --> HB
    classDef appC fill:#475569,color:#ffffff
    classDef ringC fill:#0f766e,color:#ffffff
    classDef workC fill:#1e3a8a,color:#ffffff
    classDef hbC fill:#9a3412,color:#ffffff
    class SUB,RC appC
    class SR,RR ringC
    class W workC
    class HB,FW hbC
...
|---|---|
| `start(submit_path, result_path, heartbeat_path, capacity, heartbeat_capacity)` | Opens-or-creates a `SharedRing` for both submit and result. |
| `start_with_transports(submit_ring, result_ring, heartbeat_path, heartbeat_capacity)` | Caller supplies any two `Arc<dyn MessageTransport>` (mix MPMC ring + SPMC deque freely). |
| `start_by_workload_shape(submit_path, submit_shape, result_path, result_shape, heartbeat_path, capacity, heartbeat_capacity)` | `MmfDispatcher::pick` chooses each side's family from a workload shape; returns `(Self, submit_family, result_family)`. |

`start_by_workload_shape` builds `SharedRing` (`MmfFamily::SharedRing`) or
`SharedDeque<PassSlot>` (`MmfFamily::SharedDeque(..)`) transports;
...
- **No prioritization**: the submit ring is FIFO. For
  ...
  [PriorityFanout](priority-fanout/).
- **Cross-process backed by MMF.**
...
- Source: `crates/subetha-cxc/src/scheduler.rs` (738 lines, 8 unit
  tests covering submit-execute-result round-trip,
  unknown-closure error path, args-too-large rejection,
  heartbeat registration, 16-message in-order delivery, and the
