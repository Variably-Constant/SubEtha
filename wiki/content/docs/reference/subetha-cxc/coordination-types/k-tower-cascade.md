...
[`SharedRegion`](../arenas/shared-region/); at the leaf level the
...
```mermaid
flowchart TB
    K["KTowerCascade&lt;T, 4&gt; = [u32; 4] = [top, mid1, mid2, leaf]"]
    K --> R0["Region0: [u32]"]
    K --> R1["Region1: [u32]"]
    K --> R2["Region2: [u32]"]
    K --> RL["RegionLeaf&lt;T&gt;: [T]"]
    classDef cascC fill:#1e3a8a,color:#ffffff
    classDef regC fill:#0f766e,color:#ffffff
    classDef leafC fill:#9a3412,color:#ffffff
    class K cascC
    class R0,R1,R2 regC
    class RL leafC
```
...
- Source: `crates/subetha-cxc/src/k_tower_cascade.rs` (649 lines, 15
  ...
  and disk persistence). `CascadeResolverN` also exposes
  `allocate_leaf(value) -> u32` for callers populating the
  intermediate levels by hand; `append` and `insert_at_top` are
  the full-descent helpers.
