...
[`SharedOnceCell<T>`](../cells/shared-once-cell/): the first caller's
...
block-beta
  columns 1
  cell["SharedOnceCell of T - one 64 B cache line: state atomic (EMPTY / INIT / INITIALIZED), payload (56 B for T)"]
...
- **Payload size capped at 56 bytes**: same as
  `ONCE_PAYLOAD_BYTES`. Larger T needs pointer
