...
- **`NO_LEADER = 0` is the reserved sentinel** (source line 47).
  PID 0 cannot be a real claimant; the API asserts this.
- **Global-epoch monotonic counter** (source line 55): leader (or
  any caller) advances via `tick_epoch()`. Heartbeat freshness is
  measured in epoch units, not wall-clock seconds.
- **`grace_epochs`** is the staleness window (source line 121-124):
...
- **`election_term`** increments on each successful handover
  (source line 146). Subscribers poll the term to detect leadership
  changes.
- **`step_down(my_pid)`** voluntarily releases leadership (source
  lines 199-203). Only succeeds if the caller was the leader.
- **Header layout** (source lines 49-57): magic + leader_pid +
...
- Source: `crates/subetha-cxc/src/shared_leader_election.rs` (408
  lines, multiple unit tests covering empty-claim, preemption,
