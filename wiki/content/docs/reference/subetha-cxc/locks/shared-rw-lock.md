...
- **One AtomicU64 state** (source lines 30-35): bit 63 =
...
- **Try-only / blocking APIs**: `try_read_lock` / `try_write_lock`
  are non-blocking and return `Err(WouldBlock)`; the blocking
  `read_lock` / `write_lock` spin (or yield) and return the guard
  directly (not a `Result`).
- **Header is 64 bytes** (source line 38: const assert): one
  cache line.
...
- Source: `crates/subetha-cxc/src/shared_rw_lock.rs` (507 lines).
