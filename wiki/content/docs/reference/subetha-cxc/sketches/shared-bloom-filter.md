...
[`SharedBitVec`](shared-bit-vec/): insert hashes the input k
...
- **Underlying storage**: a [`SharedBitVec`](shared-bit-vec/);
...
block-beta
  columns 1
  bh["base.bloom.bin - BloomHeader 64 B: magic, n_bits, n_hashes, padding"]
  vh["base.bits.bin - BitVecHeader 64 B (from SharedBitVec)"]
  bits["u64 x ceil(n_bits / 64): the bit array, lock-free fetch_or"]
  classDef hdrC fill:#1e3a8a,color:#ffffff
  classDef bitC fill:#0f766e,color:#ffffff
  class bh,vh hdrC
  class bits bitC
```
...
- Source: `crates/subetha-cxc/src/shared_bloom_filter.rs` (458 lines).
