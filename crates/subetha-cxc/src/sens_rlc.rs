...
/// Path-validation control frames. Outside the contiguous data range, so a
/// shared-socket demux routes them by type rather than by range.
...
impl RlcSession {
    ...
    /// established peer.
    ///
    /// Cleartext even when TLS is armed: a candidate session cannot open the
