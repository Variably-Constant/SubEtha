...
let raw_ref = (r.offset, r.len);

// Reader (any process):
let a = SharedStringArena::open("/tmp/strings.bin").unwrap();
let r = subetha_cxc::StringRef::new(raw_ref.0, raw_ref.1);
let bytes = a.get_bytes(r).unwrap();
```
...
- Source: `crates/subetha-cxc/src/shared_string_arena.rs` (564
  lines, 14 unit tests covering intern + resolve, cross-handle
