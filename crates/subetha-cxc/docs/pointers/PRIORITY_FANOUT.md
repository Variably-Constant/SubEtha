...
[`SharedRing`](./SHARED_RING.md)s (one per priority level) with
a single [`SharedAtomicU64`](./SHARED_ATOMIC.md) bitmap of
...
```text
+-----------------------------+
| SharedAtomicU64 bitmap      |   <base>.bitmap.bin
|   bit i set = ring i        |
|   non-empty (hint)          |
+-----------------------------+

+-----------------------------+  <base>.prio0.bin
| SharedRing (priority 0)     |  (lowest priority)
+-----------------------------+

+-----------------------------+  <base>.prio1.bin
| SharedRing (priority 1)     |
+-----------------------------+

...                              ...

+-----------------------------+  <base>.prio(K-1).bin
| SharedRing (priority K-1)   |  (highest priority)
+-----------------------------+
```
...
48-byte u32-tag buffer. Naive baseline: `Mutex<BinaryHeap<(u8,
...
- Source: `crates/subetha-cxc/src/priority_fanout.rs` (601 lines, 13
