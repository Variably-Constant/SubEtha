...
# Ok::<(), subetha_cxc::shared_array::ArrayError>(())
```
...
|---|---|
| A table written while it is read | [`SharedSlab`](./SHARED_SLAB.md) or [`SharedVec`](./SHARED_VEC.md): one SeqLock per slot, so a reader racing the writer retries rather than seeing a mixture. |
| Records appended at run time | [`SharedVec`](./SHARED_VEC.md): bump-pointer `push_back` with a length the readers see. |
| A caller-chosen index over records that change | [`SharedSlab`](./SHARED_SLAB.md). |
...
- Source: `crates/subetha-cxc/src/shared_array.rs`.
- Siblings: [SHARED_VEC.md](./SHARED_VEC.md), [SHARED_SLAB.md](./SHARED_SLAB.md),
  [SHARED_REGION.md](./SHARED_REGION.md).
