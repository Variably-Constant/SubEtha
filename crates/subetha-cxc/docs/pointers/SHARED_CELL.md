...
- **`open` rejects mismatched payload size**:
  opening a u64 file via `SharedCell<u32>::open` returns
  `LayoutMismatch`.
- **Durability via `flush` / `flush_async`**.
...
```text
+---------+---------+---------+---------+--------------------------+
| magic   | size    | version | pad     | payload [u8; 52]         |
+---------+---------+---------+---------+--------------------------+
  u32       u32       u32       u32       cache-line tail
```
...
- **Treating `version()` as a monotonic clock.** It is a per-cell
  ...
  requires a [SHARED_FENCE_CLOCK.md](./SHARED_FENCE_CLOCK.md) or
  similar.
...
- Sibling primitive: [SHARED_ATOMIC.md](./SHARED_ATOMIC.md) - for
  cross-process atomic ops on primitive widths. SharedAtomic
  for fetch_add semantics; SharedCell for struct snapshots.
- Sibling primitive: [SHARED_ONCE_CELL.md](./SHARED_ONCE_CELL.md) -
