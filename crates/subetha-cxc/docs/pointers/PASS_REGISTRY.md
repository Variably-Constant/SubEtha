...
| Process A                   |         | Process B                   |
|                             |         |                             |
| static REGISTRY: Lazy<RwLock|         | static REGISTRY: Lazy<RwLock|
|   id=1 -> |args| { ... }    |         |   id=1 -> |args| { ... }    |
|   id=2 -> |args| { ... }    |         |   id=2 -> |args| { ... }    |
|   id=3 -> |args| { ... }    |         |   id=3 -> |args| { ... }    |
+-------------|---------------+         +-------------|---------------+
...
- Bench: `crates/subetha-cxc/benches/pass_registry.rs` (execute,
  ...
  is_registered).
- Composes with: [PROGRESS_TASK.md](./PROGRESS_TASK.md) and
  [PRIORITY_FANOUT.md](./PRIORITY_FANOUT.md) - the
  cross-process dispatch substrates that ship `Pass { id, args }`
  payloads.
- Composes with: [OWNER_LEASE.md](./OWNER_LEASE.md) - the
