...
```text
+---------------------------+
| BitVecHeader (64B)        |  magic + capacity_bits + word_count
+---------------------------+
| AtomicU64 words           |
| word[0] = bits 0..63      |
| word[1] = bits 64..127    |
...
- Bench: `crates/subetha-cxc/benches/shared_bit_vec.rs` (set, get,
  ...
  `Mutex<Vec<u64>>`).
- Consumer:
  [SHARED_BLOOM_FILTER.md](./SHARED_BLOOM_FILTER.md) - probabilistic
  set built on top of `SharedBitVec` with `k` hash functions.
- Sibling primitive:
  [SHARED_HASH_MAP.md](./SHARED_HASH_MAP.md) - keyed map; bit-vec
  is the boolean-only specialization that drops keys entirely.
- Sibling primitive:
  [SHARED_ATOMIC.md](./SHARED_ATOMIC.md) - single-cell atomic; the
