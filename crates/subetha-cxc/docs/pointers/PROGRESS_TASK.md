...
Composes [`SharedAtomicU64`](./SHARED_ATOMIC.md) (progress
counter), `SharedAtomicU64` (total / denominator),
[`SharedAtomicBool`](./SHARED_ATOMIC.md) (done flag), and
[`SharedCell<R>`](./SHARED_CELL.md) (result payload). The work
...
```text
+-----------------------------+
| SharedAtomicU64 progress    |  <base>.progress.bin
|   Relaxed fetch_add         |
+-----------------------------+

+-----------------------------+
| SharedAtomicU64 total       |  <base>.total.bin
|   Acquire/Release           |
+-----------------------------+

+-----------------------------+
| SharedAtomicBool done       |  <base>.done.bin
|   Release publish           |
+-----------------------------+

+-----------------------------+
| SharedCell<R> result        |  <base>.result.bin
|   SeqLock; written once     |
+-----------------------------+
```
...
- Source: `crates/subetha-cxc/src/progress_task.rs` (540 lines, 12
...
- Sibling primitive: [EVENT_STATE_LOG.md](./EVENT_STATE_LOG.md) -
