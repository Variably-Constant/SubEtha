...
**Composed, not folded in.** A [SHARED_SLAB.md](SHARED_SLAB.md) of
`VersionChain<T, D>` beside a [SHARED_EPOCHS.md](SHARED_EPOCHS.md), so
the plain slab is untouched and no existing user pays for versioning.
...
### A compound write published once
...
- Substrate: [SHARED_SLAB.md](SHARED_SLAB.md) - the slab it holds,
  unchanged.
- Substrate: [SHARED_EPOCHS.md](SHARED_EPOCHS.md) - the counter, the
  pins and the tickets.
- Sibling primitive: [VERSIONED_BTREE_MAP.md](VERSIONED_BTREE_MAP.md) -
  the index side of the same snapshot.
