...
```text
+-------------------------------------+
| SharedAtomicU64 state               |   <base>.state.bin
|   bits 63..32: current epoch        |
|   bits 31..0:  arrived count        |
+-------------------------------------+

+-------------------------------------+
| External HeartbeatTable             |   passed in via Arc
|   slot i: pid + last_seen_epoch     |
+-------------------------------------+
```
...
- Source: `crates/subetha-cxc/src/epoch_barrier.rs` (539 lines, 12
...
- Dependency: [HEARTBEAT.md](./HEARTBEAT.md) - the live-peer
