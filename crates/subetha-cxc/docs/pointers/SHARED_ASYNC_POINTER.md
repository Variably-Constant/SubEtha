...
- **Speculative race waits for ALL workers.** The
...
- **Closure cloning for `get_or_speculative`.** It
  requires `Fn() -> T + Send + Sync + 'static + Clone`. The
  closure is cloned N times. Heavy state captured by the closure
  is duplicated per worker.
- **`flush_async` is sync-to-page-cache on Windows.** Real
  durable persistence requires
  `flush` (synchronous `msync`).
- **No support for non-`Copy` T.** The bound is `T: Copy + Send
  + Sync + 'static`. Heavy/large values must be stored elsewhere
  (e.g. in a `SharedRegion`) and the cell holds an offset.
