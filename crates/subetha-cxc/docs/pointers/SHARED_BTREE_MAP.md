...
Measured head-to-head on the same host and workload, its reads lost
decisively: get_hit 56.90 ns and get_miss 55.34 ns (2.84x / 1.91x
SLOWER than `Mutex<BTreeMap>`, where this tree is 1.26x / 1.44x
FASTER), because a skip-list walk pays a position-independent
`SharedRegion` lookup - a likely cache miss - per level, while the
cache-blocked tree packs its keys contiguously and binary-searches
inside one line. The skip-list implementation was removed when this
tree landed as its drop-in replacement; the same SWMR write model
and lock-free read model carry over.
...
walk it without locks. The ordering supports range and
first/min queries that a hash map cannot.
