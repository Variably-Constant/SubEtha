...
| CMSHeader (64B)           |
|   magic, d, w             |
|   total_inserts (Atomic)  |
+---------------------------+
| AtomicU64 cells           |
|   d rows x w cells each   |
|   row[i][j] counted via   |
|   pos[i] = (h1+i*h2) % w  |
+---------------------------+
```
...
for i in 0..d:
   pos_i = (h1 + i * h2) % w
   row[i][pos_i].fetch_add(1, Relaxed)
total_inserts.fetch_add(1, Relaxed)
```
...
- Source: `crates/subetha-cxc/src/shared_count_min_sketch.rs` (CMS
  primitive with FNV-1a + double-hashing, unit tests covering
  insert+estimate, overcount safety, cross-handle visibility).
